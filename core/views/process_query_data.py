...
def process_providers(df_providers):
    ...
    if not df_providers.empty:
        ...
        df_providers['empresa'] = int(company.empresa_id)
...
def process_products(df_products, integration=''):
    if not df_products.empty:
        ...
        company = get_data_company()
        df_products['empresa'] = company.empresa_id

        if integration:
            list_providers = get_providers_api(company.empresa_id)
...
def process_histories(df_histories, integration=''):
    if not df_histories.empty:
        ...
        company = get_data_company()
        df_histories['empresa'] = company.empresa_id

        if integration:
            list_providers = get_providers_api(company.empresa_id)
            list_products = get_products_api(company.empresa_id)
            list_branches = get_branches_api(company.empresa_id)
...
def process_sales(df_sales, integration=''):
    if not df_sales.empty:
        ...
        company = get_data_company()
        df_sales['empresa'] = company.empresa_id
...
def process_entries(df_entries, integration=''):
    if not df_entries.empty:
        ...
        company = get_data_company()
        df_entries['empresa'] = company.empresa_id
...
def process_orders(df_orders, integration=''):
    if not df_orders.empty:
        ...
        company = get_data_company()
        df_orders['empresa'] = company.empresa_id
...
def process_order_duplicate(df_orders):
    if not df_orders.empty:
        ...
        list_orders = get_orders_api(company.empresa_id)
        ...
        return dict_orders, company.empresa_id

    else:
        raise ValueError('Erro: consulta ao banco de dados retornou vazia!')


def process_stocks(df_stock, integration=''):
    if not df_stock.empty:
        ...
        company = get_data_company()
        df_stock['empresa'] = company.empresa_id
